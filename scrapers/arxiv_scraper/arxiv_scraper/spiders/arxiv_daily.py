...
class ArxivDailySpider(scrapy.Spider):
    name = "arxiv_daily"

    sleep_time = 3  # arXiv API recommends 3 seconds between requests
    articles_fetched = (
        0  # Counter for articles written to CSV
    )
    ...
    def __init__(self, *args, **kwargs):
        super(ArxivDailySpider, self).__init__(*args, **kwargs)
        ...
        # DAILY SPIDER: Write to a separate daily batch file, overwrite mode "w"
        results_file = os.path.join(results_dir, "daily_batch.csv")
    ...
    def is_accepted_paper(self, comment, journal_ref):
        ...
        Returns tuple (bool, str): (is_accepted, details)
        """
        ...
        if journal_ref:
            return True, f"Journal Ref: {journal_ref}"
        ...
        for pattern in acceptance_patterns:
            if re.search(pattern, comment_lower):
                return True, f"Comment match: {comment}"
        ...
        match = re.search(conf_year_pattern, comment, re.IGNORECASE)
        if match:
            return True, f"Conference match: {match.group(0)}"

        # Pattern 3: Conference + presentation type
        conf_context_pattern = (
            rf"\b({conf_pattern})\s+(camera|poster|oral|spotlight|paper)\b"
        )
        match = re.search(conf_context_pattern, comment, re.IGNORECASE)
        if match:
            return True, f"Conference context match: {match.group(0)}"

        # Pattern 4: "In [Conference]" or "At [Conference]"
        in_conf_pattern = rf"\b(in|at)\s+({conf_pattern})\b"
        match = re.search(in_conf_pattern, comment, re.IGNORECASE)
        if match:
             return True, f"In/At Conference match: {match.group(0)}"
    ...
    def parse_api_response(self, response):
        ...
        for entry in entries:
            ...
            try:
                ...
                is_accepted, acceptance_details = self.is_accepted_paper(comment, journal_ref)
                ...
                # Write to daily batch file (including acceptance info)
                self.writer.writerow(
                    [
                        arxiv_id,
                        arxiv_url,
